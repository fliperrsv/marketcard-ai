...
1. Заголовок (50-70 символов) — уникальное торговое предложение + 2-3 ключевых слова.
2. Описание (до 500 символов) — минимум 3 цифры/факта, выгоды, комплектация, призыв к действию. Описание должно быть развёрнутым и продающим.
3. Преимущества (8-10 пунктов) — конкретные, с цифрами, сравнением с конкурентами, решением болей.
4. Характеристики (12-14 пунктов) — размеры, вес, материалы, интерфейсы, напряжение, мощность и т.п.
5. Ключевые слова (14-16 фраз) — синонимы, LSI, длинный хвост, гео-запросы, запросы с "купить", "цена".

Если описание короткое, домысливай реалистичные параметры на основе типового товара.
...
`;
    ...
    const response = await axios({
      ...
      data: {
        model: 'GigaChat',
        messages: [
          { role: 'system', content: 'Ты — помощник, который возвращает только JSON-массив без пояснений. Используй английские ключи: title, description, advantages, features, keywords. Все поля всегда заполнены.' },
